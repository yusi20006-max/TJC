...
ok "$(test -f "$TJC_CONFIG_DIR/queue/items/queue_test.json"; echo $?)" "queue item persisted"
ok "$(jq -e '.status == "QUEUED" and .priority == 100' "$TJC_CONFIG_DIR/queue/items/queue_test.json" >/dev/null; echo $?)" "queue metadata valid"

set +e
tjc_queue_add_workflow "$WF" 100 queue_test >/dev/null 2>&1
CODE=$?
set -e
ok "$( [ "$CODE" -ne 0 ]; echo $? )" "duplicate queue item rejected"
